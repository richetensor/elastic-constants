...
def main(input_options, libmode=False):
	...
	# Get index specifying which ab initio program is being used.
	program = int(arguments[1])
	...
	for patt in range(numStrainPatterns/numsteps):
		...
		for a in range(0,numsteps):  
			...
			# now get corresponding stress data from the output file
                        (units, thisStress) = ab_initio.get_stress(seedname+
				"_cij__"+str(patt+1)+"__"+str(a+1),program)
